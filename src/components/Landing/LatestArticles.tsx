...
const LatestArticles: React.FC<Props> = (props) => {
  ...
  return (
    <section>
        ...
        {devData &&
          mostLikedData.map(({ description, title, slug, positive_reactions_count, published_at }) => (
            <Link key={slug} href={`/blog/${slug}`}>
              <a className="p-4 cursor-pointer border border-gray-400 hover:border-gray-600 focus:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-400 dark:focus:bg-gray-900 transition rounded-lg mt-5">
                ...
                <div className="flex justify-between mb-1">
                  <dl>
                    ...
                    <dd className="text-gray-700  transition dark:text-gray-400  text-sm  leading-6 ">
                      <time>{format(parseISO(published_at), 'MMMM dd, yyyy')} </time>
                    </dd>
                  </dl>
                  <span className="text-gray-700  transition dark:text-gray-400 text-sm flex">
                ...
                <div className="text-sm md:text-base font-normal text-gray-900  transition dark:text-gray-300">
                  {description}
                  {/* <Link href={`/blog/${slug}`}>
      ...
      <Link href="/blog">
        <a className=" flex justify-center p-3 focus:text-gray-800  transition dark:text-gray-300 dark:focus:text-white ">
